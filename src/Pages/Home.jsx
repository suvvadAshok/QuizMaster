...
const HomePage = () => {
  ...
  return (
    ...
    >
      ...
      <motion.div className="card" variants={item}>
        <img src={Balance} alt="Balance" className="w-full h-auto rounded-lg" />
        <p className="mt-2 text-center">Balances Everything</p>
      </motion.div>

      <motion.div className="card" variants={item}>
        <img src={architect} alt="Architect" className="w-full h-auto rounded-lg" />
...
<div className="mt-[50px] w-full flex max-sm:flex-col justify-between items-center max-sm:items-start">
  ...
  <Link to="/quiz" className="text-white-500  w-1/2 flex justify-end max-sm:w-full no-underline">
   <p className="px-6 pb-3 pt-3 bg-[#fbbd2d] w-32 text-right text-black font-500 rounded-xl max-sm:w-full max-sm:text-center max-sm:py-4 no-underline">  Let's start </p>
  </Link>
</div>
